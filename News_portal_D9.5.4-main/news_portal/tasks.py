...
@shared_task
def heavy_processing_task(data_list):
    def heavy_compute(item):
        total = 0
        for i in range(100000):
            total += (hash(str(item)) + i) % 12345
        return total

    results = process_in_parallel(data_list, heavy_compute, processes=None)
    logger.info(f"heavy_processing_task finished, processed {len(results)} items")
    return results

def _send_email_single(subject, html, email, user_id):
    """
    Отправка одного письма (используется в ThreadPoolExecutor).
    """
    try:
        ...
        logger.info(f"Mail sent to {email} (user {user_id})")

    except Exception as e:
        logger.exception(f"Failed sending mail to {email}: {e}")
...
def send_notify_to_subscribers(instance_id):
    ...
    recipients = [(user_id, email, username) for user_id, email, username in subcriber_list]
    ...
    for user_id, email, username in recipients:
        html = render_to_string(
            'flatpages/mail/send_html_mail.html',
            {
                'post_title': title,
                'username': username,
        ...
        tasks.append((subject, html, email, user_id))

    # Запускаем рассылку в пуле потоков
    max_workers = getattr(settings, 'MAIL_THREADPOOL_MAX_WORKERS', 8)
    logger.info(f'Using ThreadPoolExecutor with max_workers={max_workers} for mailing to {len(tasks)} recipients')

    if tasks:
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futures = [ex.submit(_send_email_single, subj, html, email, uid)
                    for subj, html, email, uid in tasks]
            for fut in futures:
                try:
                    fut.result()
                except Exception as e:
                    logger.exception(f"Mail task exception: {e}")
